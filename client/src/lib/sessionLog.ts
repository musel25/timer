import type { EntryData, Session } from './types';
...
> & { periodKey?: string | null; entry?: EntryData | null };
 ...
 * `note` records what was done. `opts.periodKey` marks which day/week/month the
 * completion counts toward (computed by the caller, which knows the timezone),
 * and `opts.entry` carries the habit template's answers.
 */
export function buildManualSession(
  ...
  opts: { periodKey?: string | null; entry?: EntryData | null } = {},
): ManualSessionInput {
